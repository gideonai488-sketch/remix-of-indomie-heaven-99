...
// Real Accra landmarks for the demo
...
const DemoTrackingPage = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // WebGL not available (e.g. headless preview) — show static fallback
      setWebGLError(true);
      return;
    }
    mapRef.current = map;
    ...
    map.on("load", async () => {
      const pickup = (await geocode(PICKUP_ADDR)) ?? [-0.207, 5.595] as [number, number];
      const delivery = (await geocode(DELIVERY_ADDR)) ?? [-0.187, 5.650] as [number, number];
      ...
      rEl.style.cssText = "font-size:30px;filter:drop-shadow(0 3px 8px rgba(0,0,0,0.6));transition:transform 0.2s;";
      rEl.textContent = "🏍️";
